# url <- "https://spotcrime.com/va/"
#
# . <- xml2::read_html(url)                           # load a webpage
# .
# . <- rvest::html_nodes(., 'a')                      # get the html link tags ('a') on the page
# .
# . <- rvest::html_attr(., 'href')                    # get the actual links(urls) from the 'href' attribute of the html 'a' tags
# .
# . <- stringr::str_match(., "/va/(.*daily$)")[,2]    # extract just the links(urls) ending in "daily"
# .
# . <- .[!is.na(.)]                                   # remove the NA entries
# .
# . <- paste0(url, .)                                 # append the starting url to the beginning of those extracted
# .
# daily_crime_report_virginia_urls <- .               # name the resulting dataset
#
#
# . <- xml2::read_html(daily_crime_report_virginia_urls[7])
# .
# . <- rvest::html_nodes(., 'a')
# . <- rvest::html_attr(., 'href')
# . <- stringr::str_match(., "/va/(.*-[0-9][0-9]$)")[,2]
# .
# . <- .[!is.na(.)]
# . <- paste0(url, .)
# .
# daily_crime_report_locality_urls <- .
#
#
# . <- xml2::read_html(daily_crime_report_locality_urls[3])
# . <- rvest::html_nodes(., 'a')
# . <- rvest::html_attr(., 'href')
# . <- stringr::str_match(., "/(mobile/crime/.*)")[,2]
# . <- .[!is.na(.)]
# . <- paste0("https://spotcrime.com/", .)
# .
# daily_crime_report_details_urls <- .
#
#
# crime_details <- xml2::read_html(daily_crime_report_details_urls[1])
#
